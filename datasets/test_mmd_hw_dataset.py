...
def display(data):
    ...
    for b in range(batchSize):
        ...
        if draw :
            ...
            cv2.imshow('img{}.png'.format(b),img.numpy().astype(np.float64))
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>3:
        ...
        repeat=1
    data=mmd_hw_dataset.MMDHWDataset(dirPath=dirPath,split='train',config={
        'img_height': 32,
        'max_width': 3000,
    ...
    dataLoader = torch.utils.data.DataLoader(data, batch_size=4, shuffle=True, num_workers=0, collate_fn=mmd_hw_dataset.collate)
